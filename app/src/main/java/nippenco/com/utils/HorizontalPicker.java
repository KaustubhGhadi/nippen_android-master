package nippenco.com.utils;

/**
 * Created by BinaryScript on 24/01/17.
 */
...
public class HorizontalPicker extends View {
    ...
    Typeface railway_bold;
    Typeface railway_regular;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (values != null) {
            for (int i = 0; i < values.length; i++) {
                ...
                textPaint.setTypeface(getTextTypeface(i));
                //manish
    ...
    private float getTextSize(int item) {
        ...
        if (scrollX > itemWithPadding * item - itemWithPadding / 2 && scrollX < itemWithPadding * (item + 1) - itemWithPadding / 2) {
            ...
            float GESTURE_THRESHOLD_DIP = 22.0f;
            float scale = getContext().getResources().getDisplayMetrics().density;
            size = (int) (GESTURE_THRESHOLD_DIP * scale + 0.5f);
        } else if(item == pressedItem) {
    ...
    //manish function
    private Typeface getTextTypeface(int item) {
        Typeface typeface;
        int scrollX = getScrollX();
        // set color of text
        int itemWithPadding = (int) (itemWidth + dividerSize);
        if (scrollX > itemWithPadding * item - itemWithPadding / 2 && scrollX < itemWithPadding * (item + 1) - itemWithPadding / 2) {
            //manish center elements
            typeface = railway_regular;

        } else if(item == pressedItem) {
            typeface = railway_bold;
        }else {
            //manish non center elements
            typeface = railway_bold;
...
//            postInvalidateOnAnimation(); // TODO
    ...
    private int getColor(int scrollX, int position) {
        ...
        if(pressedItem == position) {
            defaultColor = Color.parseColor("#FFBFBFBF");
            selectedColor = Color.parseColor("#444444");
